...
| Список тендеров | `/tenders` | 90% | TypeScript + Go tests | Работает. Поиск отправляется в backend через `keywords`; локальные вкладки и сумма фильтруют загруженную страницу. Документы ТС теперь подтягиваются также из `lotBuy.documents`. |
...
| Детальная карточка тендера | `/tenders/$tenderId` | 95% | TypeScript + Go tests | Работает с fallback-поиском по TenderPlus; исправлена выгрузка ТС через backend proxy, скачивание исходного файла и скачивание извлечённого текста. |
...
| AI-анализ лота | `/tenders/$tenderId` + RAG | 85% | TypeScript | Работает с ограничениями: поддерживает `AI_PROVIDER=groq` и Gemini fallback. Требуется ключ в `.env`; повторные одинаковые результаты берутся из локального кэша. |
| Индексация документа лота | `/tenders/$tenderId` + RAG | 90% | TypeScript + Go tests | Работает для PDF/DOC/DOCX с ручной загрузкой, автоподбором ТС, кэшем извлечённого текста и выжимки. Без OCR для сканов. |
...
| Пользователи | `/users` | 85% | TypeScript + Go tests | Страница подключена к `GET /api/v1/users` с fallback-списком; backend users зарегистрирован и мигрирует таблицу `users`. |
| Настройки | `/settings` | 75% | TypeScript | Профиль администратора и настройки уведомлений сохраняются в localStorage и показывают уведомление о сохранении. |
...
| CSV экспорт истории | `/api/v1/analytics/export` | 100% | TypeScript + Go tests | Исправлено: экспорт применяет текущие UI-фильтры истории, включая поиск, даты, суммы, регион, вид закупки, участие и исключённые лоты. |

## Backend: tenderai

| Функция | API | Уровень завершения | Проверка | Статус/заметки |
|---|---|---:|---|---|
| TenderPlus GraphQL client | `internal/tenderplus` | 90% | Go tests | Работает для списка и детального поиска; документы объединяются из `lot.documents` и `lotBuy.documents`. Зависит от внешнего API и токена. |
...
| Users API | `/api/v1/users` | 85% | Go tests | Исправлено: service зарегистрирован в `cmd/api/main.go`, таблица `users` мигрируется, frontend `/users` читает endpoint. CRUD расширение ролей остаётся будущим шагом. |
...
1. `/tenders` зависит от того, как TenderPlus интерпретирует `keywords`; если внешний API не ищет по компании, локально невозможно найти то, что не пришло в ответе.
2. `/tenders` вкладки и сумма фильтруют текущую страницу, а не весь внешний каталог TenderPlus.
3. `companies`, уведомления и справочники пока используют localStorage, поэтому не синхронизируются между пользователями.
4. `users` подключён к backend list endpoint, но роли/компании/статусы пока заполняются frontend-маппингом.
5. RAG AI-анализ требует ключ в `.env`; без него endpoints корректно возвращают 503.
6. В `main.py` остались две косметические строки ошибок `Gemini (spec summary)`; они не влияют на работу Groq, но текст можно переименовать позже.
7. Документы-сканы без текстового слоя не индексируются, потому что OCR не реализован.

## Будущие фичи в реализации

| Фича | Текущий уровень | Следующий шаг |
|---|---:|---|
| Backend CRUD для пользователей | 45% | List endpoint подключён; следующий шаг — create/update/delete, auth и roles UI. |
...
| Экспорт с текущими фильтрами | 100% | Выполнено: UI передаёт фильтры в `/analytics/export`, backend применяет общий фильтр истории. |
